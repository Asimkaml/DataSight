...
# Header
st.markdown('<p class="main-header">DataSight</p>', unsafe_allow_html=True)
        ...
        if viz_type == "Distribution Analysis":
            ...
            if st.button("Analyze", key="analyze_query"):
                with st.spinner("Processing query..."):
                    # Use AI to extract relevant columns from the query
                    mentioned_cols = st.session_state.backend._extract_columns_with_ai(query)
                    
                    if not mentioned_cols:
                        st.warning("No relevant columns identified in your query. Please try again with more specific column names.")
                    else:
                        st.info(f"Identified columns: {', '.join(mentioned_cols)}")
                        ...
                        # Handle top/bottom value queries
                        elif any(x in query.lower() for x in ["top", "highest", "max", "maximum", "lowest", "min", "minimum"]):
                            if mentioned_cols:
                                top_values_result = st.session_state.backend._handle_top_values_query(
                                    st.session_state.df, mentioned_cols[0], query
                                )
                        ...
                        # Handle time series queries
                        elif any(x in query.lower() for x in ["time", "trend", "over time", "change", "historical"]):
                            # Look for datetime columns in the mentioned columns
                            datetime_cols = []
                            ...
                            # Find numeric columns in the mentioned columns
                            value_cols = [col for col in mentioned_cols if 
                                        pd.api.types.is_numeric_dtype(st.session_state.df[col])]
                            
                            if datetime_cols and value_cols:
                                time_series_result = st.session_state.backend._handle_time_series_query(
                                    st.session_state.df, datetime_cols[0], value_cols[0]
                                )
                                st.write(time_series_result['text'])
                                if time_series_result['chart'] is not None:
                                    st.plotly_chart(time_series_result['chart'], use_container_width=True)
                        ...
                        # Fallback to general AI response if no specific pattern matched
                        else:
                            ai_response = st.session_state.backend.generate_ai_response(query)
                            st.write(ai_response['text'])
                            if ai_response['chart'] is not None:
                                st.plotly_chart(ai_response['chart'], use_container_width=True)
    with tabs[3]:
        ...
        if user_query and user_query.strip() and not st.session_state.pending_rerun:
            with st.spinner("Analyzing your question..."):
                ...
                # Use the improved AI handler
                response = st.session_state.backend.generate_ai_response(user_query)
                
                # Log performance
                st.session_state.backend._log_query_performance(
                    "ai_query",
...
<div style="text-align: center; margin-top: 3rem; padding: 1rem; border-top: 1px solid #ddd;">
    <p>DataSight | Built with Streamlit and Python</p>
</div>
""", unsafe_allow_html=True)
